...
public class ScenarioCreatorManager {
	...
	public static String[] commandList = { "End Repeat|/~endrepeat", "Sound|/~sound", "Skip|/~skip", "Pause|/~pause",
			"Repeat Button|/~repeat-button", "Repeat/~repeat", "Reset Buttons|/~reset-buttons",
			"Skip Button|/~skip-button", "Display Clear All|/~disp-clearAll", "Display Cell Pins|/~disp-cell-pins:",
			"Display String|/~disp-string:", "Display Cell Character|/~disp-cell-char:",
			"Display Cell Clear|/~disp-cell-clear:", "Display Cell Lower Pins|/~disp-cell-lowerPins:",
			"User Input|/~user-input" };

	public static String[] userCommandList = {

			"End Repeat", "Sound", "Skip", "Pause", // DONE
			"Repeat Button", "Repeat", "Reset Buttons", "Skip Button", "Display Clear All", "Display Cell Pins",
			"Display Word on Braille", // DONE
			"Display Cell Character", "Display Cell Clear", "Display Cell Lower Pins", "User Input" };
	...
	public boolean saveToFile() {
		...
		for (File f : sounds)
			try {
				Files.createDirectories(Paths.get("./AudioFiles"));
				Files.copy(f.toPath(), Paths.get("./AudioFiles/" + f.getName()), StandardCopyOption.REPLACE_EXISTING);
			} catch (IOException e) {
				...
				if (repeat) {
					...
					if (lines.get(i).length() >= 8 && lines.get(i).substring(0, 8).equals("/~sound:")) {
						...
						try {
							...
							this.sounds.add(new File(this.scenarioFile.getPath() + "/AudioFiles/" + temp.getInput()));
					...
					// The key phrase to assign a button to repeat text.
						...
						try {
							...
							temp = new RepeatButton(lines.get(i).substring(0, 16), lines.get(i).substring(16),
									this.buttonNum);
						} catch (IllegalArgumentException e) {
							this.errorMessage = e.toString();
							return false;
					...
					// will be repeated.
						...
						this.questions.get(questionIndex).getCommands()
								.add(new Repeat(lines.get(i).substring(0, 8), lines.get(i).substring(8)));
					...
					// JButtons.
						...
						this.questions.get(questionIndex).getCommands()
								.add(new ResetButtons(lines.get(i).substring(0, 15), lines.get(i).substring(15)));
					}
					...
					// The key phrase to clear the display of all of the braille cells.
						...
						this.questions.get(questionIndex).getCommands()
								.add(new DispClearAll(lines.get(i).substring(0, 15), lines.get(i).substring(15)));
					...
					else if (lines.get(i).length() >= 18
							&& lines.get(i).substring(0, 18).equals("/~disp-cell-raise:")) {

						Command temp;

						try {
							temp = new DispCellRaise(lines.get(i).substring(0, 18), lines.get(i).substring(18),
									this.cellNum);
						} catch (Exception e) {
							this.errorMessage = e.toString();
							return false;
					...
					// The key phrase to lower a pin of the specified Braille cell.
						...
						Command temp;
						try {
							temp = new DispCellLower(lines.get(i).substring(0, 18), lines.get(i).substring(18),
									this.cellNum);
						} catch (Exception e) {
							this.errorMessage = e.toString();
							return false;
					...
					// The key phrase to clear a Braille cell.
						...
						Command temp;
						try {
							temp = new DispCellClear(lines.get(i).substring(0, 18), lines.get(i).substring(18),
									this.cellNum);
						} catch (Exception e) {
							this.errorMessage = e.toString();
							return false;
					...
					} else if (lines.get(i).length() >= 21
							&& lines.get(i).substring(0, 21).equals("/~disp-cell-lowerPins")) {
						Command temp;
						try {
	...
	//
	public boolean addSound(File file) {
		try {
			this.questions.get(this.questionIndex).getCommands().add(new Sound("/~sound:", file.getName()));
	...
	public boolean addSkipButton(String jButtonIndex, String goToBtn) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new SkipButton("/~skip-button:", jButtonIndex + " " + goToBtn, this.buttonNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addRepeatButton(String jButtonIndex) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new RepeatButton("/~repeat-button:", jButtonIndex, this.buttonNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellPins(String cellPos, String charSeq) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellPins("/~disp-cell-pins:", cellPos + " " + charSeq, this.cellNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellChar(String cellPos, String cellChar) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellChar("/~disp-cell-char:", cellPos + " " + cellChar, this.cellNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellRaise(String cellPos, String pinNum) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellRaise("/~disp-cell-raise:", cellPos + " " + pinNum, this.cellNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellLower(String cellPos, String pinNum) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellLower("/~disp-cell-lower:", cellPos + " " + pinNum, this.cellNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellLowerPins() {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellLowerPins("/~disp-cell-lowerPins", ""));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	public boolean addDispCellClear(String cellPos) {
		try {
			this.questions.get(this.questionIndex).getCommands()
					.add(new DispCellClear("/~disp-cell-clear:", cellPos, this.cellNum));
		} catch (IllegalArgumentException e) {
			this.errorMessage = e.toString();
			return false;
		}
	...
	/*
	 * public static void main(String[] args) { ScenarioCreatorManager
	 * scenarioCreatorManager = new ScenarioCreatorManager(new
	 * File("Enamel/FactoryScenarios/Scenario_1.txt"));
	 * 
	 * scenarioCreatorManager.parseFile();
	 * 
	 * System.out.println(scenarioCreatorManager.toString()); }
	 */
	// The following example recreated the Scenario_1.txt under the file name
	// Scenario_10.txt
	// Tunning this main method will create a Scenario_10.txt which will be exactly
	// the same as Scenario_1.txt
	/*
	 ...
	 * ScenarioCreatorManager scenarioCreatorManager = new
	 * ScenarioCreatorManager(temp);
	 * 
	 * // You MUST set cell and button num before adding any commands. // Title
	 * command is optional scenarioCreatorManager.setCellNum(1);
	 * scenarioCreatorManager.setButtonNum(4);
	 * scenarioCreatorManager.setTitle("Directional Orientation");
	 * 
	 * // Every method for every command returns a boolean which tells you whether
	 * the input was valid or not //When it returns false it will update the
	 * errorMessage string which can be accessed with getErrorMessage();
	 * 
	 * if (!(scenarioCreatorManager.addDispCellPins("0", "11100000"))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * if (!(scenarioCreatorManager.addText(Arrays.
	 * asList("These are pins 1, 2 and 3, the 3 pins on the left side. ",
	 ...
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * // Use next question to increment the question index and put a space in the
	 * ScenarioFile for the next question scenarioCreatorManager.nextQuestion();
	 * 
	 ...
	 * 
	 * if (!(scenarioCreatorManager.addText(Arrays.
	 * asList("These are pins 4, 5 and 6, the 3 pins on the right side. ",
	 * "Press button 1 to continue.")))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * if (!(scenarioCreatorManager.addSkipButton("0", "ONEE"))) {
	 ...
	 * 
	 * if (!(scenarioCreatorManager.addText(Arrays.
	 * asList("These are pins 1 and 4, the two pins on the top. Press button 1 to continue."
	 * )))) { System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * if (!(scenarioCreatorManager.addSkipButton("0", "ONEE"))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 ...
	 * 
	 * if (!(scenarioCreatorManager.addText(Arrays.
	 * asList("These are pins 3 and 6, the two pins on the bottom. Press button 1 to continue."
	 * )))) { System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * if (!(scenarioCreatorManager.addSkipButton("0", "ONEE"))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 ...
	 * 
	 * if (!(scenarioCreatorManager.addText(Arrays.
	 * asList("That's the end of directional orientation")))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
	 * 
	 * if (!(scenarioCreatorManager.addDispCellClear("0"))) {
	 * System.out.println(scenarioCreatorManager.errorMessage); }
