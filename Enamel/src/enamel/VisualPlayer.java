...
public class VisualPlayer extends Player {
	...
	/**
	 * Initializes the parameters for a window with <code>brailleCellNumber</code>
	 * Braille cells and <code>jbuttonNumber</code> buttons. The two parameters must
	 ...
	 * parameters based on the input. To set the number of cells and buttons on the
	 * player the constructor <code> VisualPlayer(int, int) </code> should instead
	 * be used
	 * 
	 * @param buttonNumber
	 */
	...
	/**
	 * Returns a reference to the button at the index passed as argument. The main
	 * purpose of providing this method is so the client can add actionListeners to
	 * the button. Buttons are numbered from left to right as they appear in the
	 * frame, from 0 to (bbuttonNumber-1), bbuttonNumber being the number of buttons
	 ...
	 * @throws IllegalArgumentException
	 *             if the index is negative or equal to or bigger than buttonNumber
	 *             (the number of buttons initialized)
	 */
	 ...
	 * BrailleCell object. For the VisualPlayer class, this method loops through the
	 * jRadioButton array and sets the corresponding jRadioButton pins to
	 * <code>setSelected(true)</code> or <code>setSelected(false)</code>, matching
	 * it with the brailleList's BrailleCell object's current state of boolean pins.
	 */
	@Override
	public void refresh() {
	 ...
	 * <p>
	 * The actionPerformed method requires a reference to the ScenarioParser, needed
	 * for it to call ScenarioParser.skip() method as well as have access to the
	 * userInput field. Pressing this key will skip to the specified area in the
	 * scenario file.
	 * 
	 * @param index
	...
	/**
	 * Adds an ActionListener to the JButton at the specified index of buttonList.
	 * The index must be between 0 and buttonNumber. The ActionListener requires a
	 * reference of the ScenarioParser, needed for it to call
	 * <code>ScenarioParser.repeatText()</code> method as well as have access to the
	 * userInput field. Pressing this key will repeat the speech of the text
	...
	/**
	 * Removes the ActionListener from the button at specified index of buttonList
	 * passed as argument. The index must be between 0 and buttonNumber.
	 * 
	 * @param index
