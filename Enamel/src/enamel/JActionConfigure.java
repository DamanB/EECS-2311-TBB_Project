...
public class JActionConfigure extends JPanel {
	...
	private void createGUI() {
		JActionConfigure.panelSize = new Dimension((int) (ScenarioCreatorGUI.configuration.getSize().width * 0.9),
				(int) (ScenarioCreatorGUI.configuration.getSize().height * 0.9));
		configScroll = new JScrollPane(main, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
	...
	public class Checkpoint extends Action {
		public Checkpoint() {
			...
			instruction.setText(
					"Checkpoint: Give this checkpoint a name (One word consiting of only letters and no duplicates)");
		}
	...
	public class JDisplayWordInBraille extends Action {
		...
		private JDisplayWordInBraille() {
			...
			instruction.setText(
					"Display Word: Please enter the word you would like to display in braille. Ensure adequate Braille cells are created beforehand");
	...
	public class JRepeat extends Action {
		...
		private JRepeat() {
			...
			instruction.setText(
					"Repeat Text: Set the text to repeat, as well as the button required to initalize the repeat");
			stringToDisp = new JTextField();
	...
	public class GoToCheckpointButtonClick extends Action {
		...
		private GoToCheckpointButtonClick() {
			...
			instruction.setText(
					"Go To Checkpoint: Select the index of the checkpoint you would like to travel to when a specfifc button is clicked. The checkpoint must occur after this one");
		...
		public boolean build(ScenarioCreatorManager sm) {
			...
			return sm.addSkipButton(Integer.toString(buttons.value()),
					ScenarioCreatorGUI.nodes.get(nodeIndex.value()).getCheckpointName().toUpperCase());
		}
	}
	...
	public class JDisplayChar extends Action {
		...
		private JDisplayChar() {
			...
			instruction.setText(
					"Display Character: Enter the character you would like to display as well as the index of cell it will appear on");
			stringToDisp = new JTextField();
	...
	public class GoToEvent extends Action {
		...
		private GoToEvent() {
			...
			instruction.setText(
					"Go To Event: Select which event you would like to travel to. The event must occur after this one");
	...
	public class JResetButtons extends Action {
		public JResetButtons() {
			...
			instruction.setText(
					"Reset Buttons: Resets the commands implemented with each button. Recommended to use at the beggining of each event");
		}
	...
	public class JLowerPins extends Action {
		...
		Integer[] pin = { 1, 2, 3, 4, 5, 6, 7, 8 };

		private JLowerPins() {
			super.name = "Braille: Lower Specific Pin";
			instruction.setText(
					"Lower Pin on Cell: Select which Braille Cell you want to edit then select the pin to lower");
			main.add(new JLabel("Braille Cell: "));
			cells = new BrailleCellSpinner();
			main.add(cells);
	...
	public class JRaisePins extends Action {
		...
		private JRaisePins() {
			...
			instruction.setText(
					"Lower Pin on Cell: Select which Braille Cell you want to edit then select the pin to raise");
			main.add(new JLabel("Braille Cell: "));
			cells = new BrailleCellSpinner();
			main.add(cells);
			pins = new JComboBox<Integer>(pin);
